...
//        {
        ...
        User a = dao.checkAccountExist(user);
         if(a!=null){
            ...
            request.getRequestDispatcher("addAdmin").forward(request, response); //DUng khi cần đẩy dữ liệu sang trang khác
        
         }
        else if(!pass.equals(repass)){
             request.setAttribute("mess1", "Re-password not the same with password!!!");
            request.getRequestDispatcher("addAdmin").forward(request, response); //DUng khi cần đẩy dữ liệu sang trang khác
        }
        //signup
        else{
            dao.addAdmin(user, pass, name, phone, address, securityQuestion, answer);
